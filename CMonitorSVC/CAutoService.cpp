...
void CAutoService::Start()
{
	...
	if (stat("C:\\Program Files (x86)\\RM\\Client", &info) != 0) {
		_mkdir("C:\\Program Files (x86)\\RM\\Client");
	}
	GetServerIP();
	if (bActive == false) {
		StartSocket();
...
bool CAutoService::RunProcess()
{
	...
	wsprintf(m_wcsProcessPath, L"C:\\Program Files (x86)\\RM\\Client\\Monitor.TaskView.exe");
...
DWORD WINAPIV CAutoService::Receive(LPARAM lpParam)
{
	...
	FILE *fp;
	fopen_s(&fp, "C:\\Program Files (x86)\\RM\\Client\\Monitor.TaskView.exe", "wb");
	bFlag = false;
	while (1)
	{
...
DWORD CAutoService::FindProcessID()
{
	...
	WCHAR processName[22] = L"Monitor.TaskView.exe";
...
int CAutoService::GetRegisterVersion()
{
	...
	DWORD dwIPSize = sizeof(IP);
	DWORD dw = sizeof(DWORD), dwData = 0;
	const char* subkey = "Ryonbong\\AutoService";
...
int CAutoService::SetRegisterVerion(char * version)
{
	...
	RegCreateKeyExA(HKEY_CURRENT_USER, "Ryonbong\\AutoService", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hk, &dwDisp);
